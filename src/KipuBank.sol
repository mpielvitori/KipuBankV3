 ...
 * @title KipuBank
 * @dev A multi-token bank contract supporting deposits of ETH and any ERC20 token with direct USDC pairs on Uniswap V2.
 *      All deposits are automatically converted to USDC for unified storage and accounting. Withdrawals are always in USDC.
 * @notice This contract demonstrates advanced Solidity patterns including:
 *         - Multi-token deposit support with automatic USDC conversion via Uniswap V2
 *         - Role-based access control (Admin/Operator roles)
 *         - OpenZeppelin Pausable integration for emergency stops
 *         - Reentrancy protection and comprehensive input validations
 *         - ETH vs ERC20 handling with clear separation of concerns
 */
contract KipuBank is ReentrancyGuard, AccessControl, Pausable {
     ...
     *         Use this function for native ETH only. For WETH or other ERC20 tokens, use depositTokenAsUSD().
     */
